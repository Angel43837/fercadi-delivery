...
- `AppConstants.supabaseServiceRoleKey` debe estar **vacía en git** — solo completarla localmente si es necesario
- La clave `service_role` nunca debe ir en código cliente; usar Supabase Edge Functions para operaciones admin
- `rider_stats` solo debe permitir lecturas desde el cliente; las escrituras solo via RPC con `SECURITY DEFINER`
- `/flota` requiere autenticación con rol `jefe_flota` — no es ruta abierta
